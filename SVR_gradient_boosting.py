...
class SvrEstimetor(object):
    def __init__(self,svr_params=None):
        self.svr_params=svr_params

    def fit(self, X, y, sample_weight=None):
        self.svr = SVR()
        if self.svr_params != None:
            self.svr.set_params(self.svr_params)
        self.svr.fit(X, y)

    def predict(self, X):
        y = self.svr.predict(X)
        return np.reshape(y, (X.shape[0], 1))
...
class BaseGradientBoosting(six.with_metaclass(ABCMeta, BaseEnsemble)):
    ...
    def __init__(self, loss, learning_rate, n_estimators, criterion,
                 ...
                 warm_start=False, presort='auto', svr_parms=None):
    ...
    def _init_state(self):
        ...
        if self.init is None:
            ...
            if self.init == 'svr':
                self.init_ = INIT_ESTIMATORS[self.init](self.svr_params)
            else:
...
class GradientBoostingClassifier(BaseGradientBoosting, ClassifierMixin):
    ...
    def __init__(self, loss='deviance', learning_rate=0.1, n_estimators=100,
        ...
        super(GradientBoostingClassifier, self).__init__(
            ...
            presort=presort, svr_parms=svr_parms)
